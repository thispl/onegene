...
def validate_attached_file(file, records, job_id):
    ...
    for idx, pp in enumerate(records, start=1):
        try:
            cust_no, so_number, item, schedule_month = pp[0], pp[1], pp[2], (pp[3] or "").strip()
            s_qty = pp[4]
            tp_1 = pp[5]
            tp_2 = pp[6]

            if not cust_no or not frappe.db.exists("Customer", {"customer_code": cust_no}):
                error_logs.append(f"Row {idx}: Customer '{cust_no}' does not exist.")
                continue

            if not so_number or not frappe.db.exists("Sales Order", so_number):
                error_logs.append(f"Row {idx}: Sales Order '{so_number}' does not exist.")
                continue

            sales = frappe.get_doc("Sales Order", so_number)

            if sales.customer_order_type != "Open":
                error_logs.append(f"Row {idx}: Sales Order '{so_number}' is not of type 'Open'.")
                continue

            so_item = next((i for i in sales.items if i.item_code == str(item)), None)
            if not so_item:
                error_logs.append(f"Row {idx}: Item '{item}' is not part of Sales Order '{so_number}'.")
                continue
            ...
            exists = frappe.db.exists("Sales Order Schedule", {
            ...
            if exists:
                error_logs.append(f"Row {idx}: Schedule already exists for {so_number}, {item}, {schedule_month}.")
                continue

            valid_records.append([cust_no, so_number, item, schedule_month, s_qty, tp_1, tp_2])
...
def validate_attached_file_return_errors(file, records):
    ...
    for idx, pp in enumerate(records, start=1):
        ...
        try:
            cust_no, so_number, item, schedule_month = pp[0], pp[1], pp[2], (pp[3] or "").strip()
            s_qty, tp_1, tp_2 = pp[4], pp[5], pp[6]

            # ✅ Validate Customer
            if not cust_no or not frappe.db.exists("Customer", {"customer_code": cust_no}):
                row_errors.append(f"Customer '{cust_no}' does not exist.")

            # ✅ Validate Sales Order
            if not so_number or not frappe.db.exists("Sales Order", so_number):
                row_errors.append(f"Sales Order '{so_number}' does not exist.")
            else:
                sales = frappe.get_doc("Sales Order", so_number)

                if sales.customer_order_type != "Open":
                    row_errors.append(f"Sales Order '{so_number}' is not of type 'Open'.")
                
                so_item = next((i for i in sales.items if i.item_code == str(item)), None)
                if not so_item:
                    row_errors.append(f"Item '{item}' is not part of Sales Order '{so_number}'.")
            ...
            if exists:
                # warning.append(f"Schedule already exists for {so_number}, {item}, {schedule_month}.")
                exists_val = frappe.db.get_value("Sales Order Schedule",{"sales_order_number": so_number,"schedule_month": schedule_month,"item_code": item, "docstatus": 1},"delivered_qty")
                if exists_val and exists_val > s_qty:
                    row_errors.append(f"While Revising the Schedule for {so_number}, {item}, {schedule_month}, the schedule qty{s_qty} is less than delivered qty{exists_val}.")
