...
def enqueue_update_stock_qty_in_job_card(doc):
	try:
		for stock_entry_details in doc.items:
			job_cards = frappe.db.get_all("Job Card Required Material Operation", {"item_code": stock_entry_details.item_code, "parent": ["!=", doc.job_card]}, pluck="parent")
			for job_card_name in job_cards:
				job_card = frappe.get_doc("Job Card", job_card_name)
				job_card.save(ignore_permissions=True)
				frappe.errprint("dweknfewkjnfkjwenkjfwen")
