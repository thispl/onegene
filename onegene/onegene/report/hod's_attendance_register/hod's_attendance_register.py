	...
	for emp in employees:
		...
		if user:
			...
			if ("HOD" in user_roles):
				...
				total_allow = 0
				for date in dates:
					att = frappe.db.get_value("Attendance",{'attendance_date':date,'employee':emp.name},['status','in_time','out_time','shift','employee','attendance_date','custom_total_working_hours','custom_overtime_hours','custom_late_entry_time','leave_type','custom_early_out_time','late_entry','early_exit']) or ''
					if att:
