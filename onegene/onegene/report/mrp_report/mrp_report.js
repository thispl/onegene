...
frappe.query_reports["MRP Report"] = {
	"filters": [
		{
			"fieldname":"from_date",
			"label": __("From Date"),
		...
		{
			"fieldname":"highlighted_rows",
			"label": __("Highlighted Rows"),
			"fieldtype": "Check",
			"description": 'Actual Stock < Safety Stock'
		},
		{
			"fieldname":"run_mrp",
			"label": __("Run MRP"),
	...
	formatter: (value, row, column, data, default_formatter) => {
		...
		if ((column.fieldname == "item_code"|| column.fieldname == "item_type"|| column.fieldname == "item_name"|| column.fieldname == "uom") && data.safety_stock > data.actual_stock_qty && data.item_type!='Process Item') {
		...
		if (column.fieldname == "expected_date" && data ) {
			...
			column.link_onclick = "frappe.query_reports['MRP Report'].set_route_to_allocation(" + JSON.stringify(data) + ")";
		}
		if (column.fieldname == "actual_stock_qty" && data ) {
			value = data["actual_stock_qty"];			
			column.link_onclick = "frappe.query_reports['MRP Report'].set_route(" + JSON.stringify(data) + ")";
		}
		if (column.fieldname == "required_qty" && data ) {
			value = data["required_qty"];			
			column.link_onclick = "frappe.query_reports['MRP Report'].set_route_to_req(" + JSON.stringify(data) + ")";
		}
		if (column.fieldname == "cover_days" && data ) {
			value = data["cover_days"];			
			column.link_onclick = "frappe.query_reports['MRP Report'].set_route_to_cover_days(" + JSON.stringify(data) + ")";
		}
	...
	"set_route_to_req": function (data) {
		...
		frappe.db.get_value("Material Planning Details", {'item_code':data["item_code"],'date':frappe.datetime.get_today()},['name']).then(exists => {
			if (exists.message.name) {
				frappe.call({
					method:"onegene.onegene.report.mrp_report.mrp_report.mpd_details",
					args: {
	...
	"set_route_to_cover_days": function (data) {
		...
		frappe.call({
			method:"onegene.onegene.report.mrp_report.mrp_report.mpd_cover_days",
			args: {
				"cover_days":data["cover_days"],
			},
			callback(r) {
	...
	"set_route": function (data) {
		...
		frappe.call({
			method:"onegene.onegene.report.mrp_report.mrp_report.stock_details_mpd_report",
			args: {
				"item":data.item_code,
			},
			callback(r) {
	...
	onload: function(report) {
		...
		report.page.add_inner_button(__("Generate Material Request"), function() {
			frappe.call({
				method: "onegene.onegene.report.mrp_report.mrp_report.return_item_type",
				callback: function(r) {
					if (r && r.message) {
						let options = r.message
							.filter(df => df.item_type !== 'Process Item')
							.map(df => ({
						...
						let dialog = new frappe.ui.Dialog({
							...
							primary_action: () => {
								...
								if (values) {
									...
									frappe.call({
										method: "onegene.onegene.report.mrp_report.mrp_report.return_print",
										args: {
