...
def get_columns():
	columns = [
		...
		{"label": _("Out Time"), "fieldname": "out_time", "fieldtype": "Data", "width": 170},
		{"label": _("Early Out Time"), "fieldname": "in_time", "fieldtype": "Data", "width": 170},
	]
	return columns
	...
	for att in attendance:
		if att.shift and att.out_time:
			...
			if att.out_time < datetime.combine(att.attendance_date, shift_end_time):
				row = [att.employee, att.employee_name,att.custom_employee_category, att.department, frappe.utils.data.format_date(att.attendance_date), att.shift, shift_end_time, att.out_time,end_time - att.out_time]
				data.append(row)
	return data
