...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		if filters.get("batch_no") or inventory_dimension_filters_applied:
			...
			sle.update({"qty_after_transaction": actual_qty if actual_qty != 0 else "" , "stock_value": stock_value})

		# sle.update({"in_qty": max(sle.actual_qty, 0), "out_qty": min(sle.actual_qty, 0)})
		sle.update({"in_qty": max(sle.actual_qty, 0) if max(sle.actual_qty, 0) != 0 else "", "out_qty": min(sle.actual_qty,0) if min(sle.actual_qty,0) !=0  else ""   })
...
def get_opening_balance(filters, columns, sl_entries):
	...
	row = {
		"item_code": _("'Opening'"),
		"qty_after_transaction": last_entry.get("qty_after_transaction", 0) or None ,
