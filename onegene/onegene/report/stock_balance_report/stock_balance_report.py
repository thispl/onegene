	...
	def prepare_new_data(self):
		...
		for key, report_data in self.item_warehouse_map.items():
			...
			report_data.update(
               ...
               {
				"reserved_stock": "" if flt(sre_details.get((report_data.item_code, report_data.warehouse), 0.0), self.float_precision) == 0 else sre_details.get((report_data.item_code, report_data.warehouse), 0.0),
                "in_qty": "" if flt(report_data.in_qty, self.float_precision) == 0 else report_data.in_qty,
                "out_qty": "" if flt(report_data.out_qty, self.float_precision) == 0 else report_data.out_qty,
                "bal_qty": "" if flt(report_data.bal_qty, self.float_precision) == 0 else report_data.bal_qty,
                "opening_qty": "" if flt(report_data.opening_qty, self.float_precision) == 0 else report_data.opening_qty
