...
def get_columns_for_leave_types() -> List[Dict]:
	types=[]
	ltypes = ['Casual Leave','Compensatory Off','Earned Leave','Leave Without Pay','Marriage leave','Medical Leave','Sick Leave']
	for entry in ltypes:
		types.append(
				...
				if attendance_on_day == 'On Leave':
					...
					if hh :
						...
						if frappe.db.exists("Attendance Request",{"employee":emp.name,"from_date":day,"reason":"On Duty","docstatus":1}):
							status="OD"
						elif frappe.db.exists("Attendance Permission",{"employee":emp.name,"permission_date":day,"docstatus":1}):
							status='P/PR'
						else:
							status="P"
