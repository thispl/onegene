...
def move_old_items_stock(old_item, new_item):
	...
	stock_entry.company = frappe.db.get_value("Warehouse", warehouses[0], "company")
...
def get_price_history_html(item_code):
	...
	"""
	idx = 0
	if not sales_order_schedules:
		html += f"""<td colspan=6 class="pt-2 pb-2 pr-4 text-center">No Sales History</td>"""

	precision = frappe.get_precision("Item", "custom_last_sales_rate") + 1
	...
	precision = frappe.get_precision("Item", "custom_rm_cost") + 1
	...
	for idx, row in enumerate(reversed(item_doc.custom_rm_cost_revisions), start=1):
		...
		html += f"""
			...
			<td class="pt-2 pb-2 pl-3 text-right">{round(flt(row.rm_cost), precision)}</td>
			<td class="pt-2 pb-2 pr-2 text-right">{round(flt(row.revised_rm_cost), precision)}</td>
		</tr>
	"""
	html += """
		</table>
