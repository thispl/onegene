...
def update_title_in_mr(doc, method):
    ...
    doc.custom_fiscal_year = get_year_code()
...
def update_quantity_in_job_card(doc, method):
    ...
    if len(doc.custom_required_material_for_operation) > 0:
        ...
        for row in doc.custom_required_material_for_operation:
            ...
            if row.idx == 1:
                possible_qty = row.possible_production # If there is only one RM in a Job Card
            else:
                possible_qty = min(row.possible_production, temp_possible) # tem_possible -> previous row's possible_production
            temp_possible = row.possible_production
    
        if operation_count > 1:
            possible_qty = min(possible_qty, doc.for_quantity)
            possible_qty = math.floor(possible_qty) - (flt(doc.custom_rework_waiting_qty_details) - flt(doc.custom_rework_waiting_qty)) # if rework qty is there, reduce from possible qty
...
def create_pending_open_order():
    ...
    for i in so_list:
        ...
        new_doc.save(ignore_permissions=True)
@frappe.whitelist()
def get_employees_before_cutoff():

    import frappe
    from frappe.utils import today, add_days, getdate

    today_date = getdate(today())
    employees_before_cutoff = []
    ...
    for emp in employees:
        ...
        # get company holiday list first
        company_holiday = frappe.db.get_value(
            "Company", emp.company, "default_holiday_list"
        )

        holiday_list = company_holiday or emp.holiday_list
        ...
        while working_days < 20:
            ...
            check_date = add_days(today_date, -pointer)
            pointer += 1
        ...
        if emp.date_of_joining and getdate(emp.date_of_joining) <= cutoff_date:
            employees_before_cutoff.append(emp.name)

    return employees_before_cutoff
@frappe.whitelist()
def update_status_inactive():
    ...
    employees = get_employees_before_cutoff()
    ...
    for emp in employees:
        ...
        while working_days < required_days:

            check_date = add_days(today_date, -day_back)
            day_back += 1
            working_days += 1
            ...
            la = frappe.db.sql("""
                ...
                AND attendance_date = %s
                AND docstatus != 2
                AND (leave_application IS NOT NULL OR attendance_request IS NOT NULL) 
                LIMIT 1
            """, (emp, check_date))
...
def trigger_mail_for_inactive():

    emp_list = update_status_inactive()
    ...
    for emp in employees:
        ...
        # emp_doc.status='Inactive'
        # emp_doc.custom_inactive_date=getdate(today())
        # emp_doc.save()
    ...
    role_user_ids = list({d.parent for d in role_users})

    role_emails = frappe.get_all(
        "User",
        filters={"name": ["in", role_user_ids]},
        pluck="email"
    )

    for dept, emp_data in dept_map.items():

        hod = frappe.db.get_value("Department", dept, "custom_hod")

        hod_email = None
        if hod:
            hod_email = frappe.db.get_value("Employee", hod, "company_email")

        recipients = role_emails.copy()
        ...
        message = f"""
        <p>Inactive Employees in <b>{dept}</b></p>
        ...
        frappe.sendmail(
            ...
            subject=f"Inactive Employees - {dept}",
            message=message
        )
...
def trigger_mail_for_inactive():
    ...
    for dept, emp_data in dept_map.items():
        ...
        for e in emp_data:
            ...
            if last_checkin and last_leave:
                recent_date = max(last_checkin[0].attendance_date, last_leave[0].attendance_date)

            elif last_checkin:
                recent_date = last_checkin[0].attendance_date

            elif last_leave:
                recent_date = last_leave[0].attendance_date
            rows += f"""
                <tr>
                    <td>{sr_no}</td>
                    <td>{e.name}</td>
        ...
        message = f"""
        <b>Dear HR & HOD's Team,</b> <br><br>

        This is to inform you that the below employees are marked as inactive due to 20
        days continuous absence.<br><br>
